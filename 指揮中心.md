...
**成本優化機制：**
- 客套話短路（route.js QUICK_REPLIES）
- 智慧截斷對話歷史（> 6 則時壓摘要）
- 趨勢摘要降頻 5→15 次、旅程摘要每天最多 1 次
...
- 累積式：前一版旅程 + 近 20 標籤 + 摘要 + 里程碑 → AI 產生更新版
...
| 功能 | 說明 |
|------|------|
| 食物分類測驗 | 178 題，8 大分類，4 等級（食物新手→食物博士），優先出新題 |
| 瘦身知識大挑戰 | 270 題是非題，零 token 消耗 |
| 健康存摺 | streak 連續互動天數 |
...
| `/api/admin/reset-user` | 用戶資料重置（清除 Redis 14 key + Supabase 8 表） |
