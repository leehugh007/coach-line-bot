...
2. 點出她自己可能沒注意到的改變——「你可能覺得沒什麼，但⋯⋯」
3. 用具體的行為說她的好，不要用形容詞（「你會把零食買了放著不一定吃」比「你很有自制力」好一百倍）
4. 最後加一段「小幫手想跟你說」——用最真誠的一段話收尾，像朋友私底下跟她說的那種

【語氣——超級重要】
你的口氣要像一個很熟的朋友在跟她聊天。輕鬆、真誠、有時候帶點幽默。
不是在寫報告、不是在做心理諮商、不是在頒獎。
就是一個真的很了解她的朋友，想讓她知道「欸我覺得你其實很不錯耶」。

好的口氣：「說真的」「其實你知道嗎」「這個你自己可能沒感覺」「欸但你有沒有發現」
壞的口氣：「展現了」「令人敬佩」「積極的態度」「堅實的基礎」「值得肯定」

禁止：AI 分析腔、心理學術語、考績評語、心靈雞湯、markdown 格式符號（# ** - 都不要）。

【格式】
- 用「你」稱呼，直接叫名字不加「學員」
- 3-4 段自然段落，不要用列點或標題
- 總共 250-350 字（不要硬湊，但不用省著寫）
- 最後一段是「小幫手想跟你說」的真心話
- 直接開始寫，不要任何開場白或標題`;
...
export async function GET(request) {
  ...
  try {
    ...
    const [
      ...
      sb.from('milestones').select('content, type, detected_at').eq('user_id', userId).order('detected_at', { ascending: false }),
    ...
    return NextResponse.json({
      ...
      selfChecks: selfCheckRes.data || [],
      goals: goalsRes.data || [],
      milestones: (milestonesRes.data || []).map(m => ({ ...m, created_at: m.detected_at })),
      stats: {
        activeDays,
