...
export async function generateDraftResponse(userText, studentContext = '', userId = null, groupContext = []) {
  try {
    ...
    let contextBlock = '';
    if (groupContext.length > 0) {
      const recent = groupContext.slice(-8);
      contextBlock = `\n\n【群組最近的對話（幫助你理解這個問題的脈絡）】\n` +
        recent.map(m => `${m.name}：${m.text}`).join('\n');
    }
    ...
    const draft = sanitizeOutboundLinks(stripMarkdown(rawDraft));
...
- 「這個轉變是最重要的」
- 「這句話說得很對」
直接 tag 回應學員的某個細節，不要先說「我看到了」「我有注意到」
 ...
 * @returns {string|null} 草稿回覆
 */
export async function generateAchievementDraftResponse(userText, studentContext = '', userId = null, groupContext = []) {
  try {
