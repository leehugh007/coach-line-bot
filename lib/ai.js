 ...
 * 用關鍵字啟發式篩選，省 AI 成本
 */
export function isCoachableQuestion(text) {
  if (!text || text.length < 20 || text.length > 2000) return false;

  const keywords = [
    // 心態困擾
    '怎麼辦', '好難', '好煩', '好累', '撐不住', '想放棄',
    '自責', '焦慮', '難過', '沮喪', '壓力', '失控',
    '沒信心', '沒動力', '做不到', '堅持不了',
    // 飲食問題
    '怎麼吃', '可以吃', '能不能吃', '該吃什麼',
  ...
  // 至少命中 1 個關鍵字，且訊息夠長（不是隨便打招呼）
  return matchCount >= 1 && text.length >= 30;
}
...
export function classifyQuestion(text) {
  ...
  if (['自責', '焦慮', '難過', '壓力', '想放棄', '撐不住', '沒信心', '沒動力'].some(k => lower.includes(k))) return 'mindset';
  if (['怎麼吃', '可以吃', '外食', '份量', '熱量', '零食', '宵夜', '吃太多', '暴食', '破戒'].some(k => lower.includes(k))) return 'diet';
  if (['體重', '卡住', '停滯', '沒瘦', '變胖', '復胖'].some(k => lower.includes(k))) return 'plateau';
  if (['被說胖', '被笑', '被念', '家人說', '朋友說'].some(k => lower.includes(k))) return 'emotion';
  return 'other';
}
...
export async function generateDraftResponse(userText, studentContext = '') {
  const draftInstruction = `你現在要幫教練寫一份「草稿回覆」。這份草稿會由教練本人複製後，到 LINE 群組中 tag 學員回覆。

【重要】
- 簡短有力，2-3 段，每段 1-2 句，總共約 200 字
- 開頭可以直接回應學員的問題，不需要「嗨」「你好」
- 保持一休教練的溫暖直接風格
- 結尾給一個可以馬上做的行動建議
- 這是群組公開回覆，語氣要讓其他學員看了也受益

${studentContext ? `【這位學員的資訊】\n${studentContext}\n` : ''}
【學員在群組中說的話】
「${userText}」
...
只輸出草稿回覆文字，不要任何其他說明。`;
  ...
  try {
    ...
    if (!res.ok) {
      console.error('[AI] Draft error:', res.status);
      return null;
    }
