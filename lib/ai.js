...
export async function aiDetectQuestion(text, groupContext = [], userId = null) {
  ...
  const prompt = `你是「ABC 減肥課程」群組的訊息分類器。判斷這則訊息是否是學員「帶著疑問、不知道怎麼做」的問題，值得通知教練回應。

【判斷原則——精準但不要漏掉好的出場機會】
教練（休校長）想親自回覆學員的問題，學員也喜歡教練出場。
判斷標準：這則訊息「教練用他的視角回覆，會不會讓學員覺得被重視、有收穫」。
帶著疑問的、不知道怎麼做的、需要觀念引導的 → 通知。
不確定的就判 false，但真的有在問問題的不要漏掉。

【通知教練的（isQuestion = true）— 必須是「有疑問」的訊息】
...
【不通知的（isQuestion = false）】
- 有人在「回答」別人的問題（看上下文：前一則是問題，這則在給建議/解答 → 這是回答者，不是提問者）
  回答者的語氣特徵：「你可以...」「建議...」「這樣很好」「試試看...」「我覺得你可以...」
  即使回答中帶有問句（「那這樣碳水要怎麼算？」），如果是在回應前一則問題，就是回答者在延伸討論，不是新的提問
- 純分享/打卡：「今天有運動」「吃了XX」「今天走了8000步」
- 純抱怨但沒有問問題：「好累」「今天又爆了」「壓力好大」（沒有「怎麼辦」「該怎麼做」）
- 正面分享：「瘦了2公斤好開心」「今天菜吃很多」
- 閒聊附和：早安、加油、好棒、+1、收到、了解、謝謝、我也是
- 回覆別人/討論別人的事（看上下文，如果是在回應前一則訊息 → false）
- 自我介紹（新學員打招呼介紹自己）
- 課程行政：幾點上課、怎麼請假、怎麼上傳
- app/系統操作：「照片搞亂了」「AI 怎麼了」
- 學員之間互相聊天、互相鼓勵（不是在問問題）
${contextBlock}
【當前要判斷的訊息】
「${text}」
