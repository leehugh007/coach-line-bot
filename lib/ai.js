...
【回覆長度——超級重要，請嚴格遵守】
這是手機上的 LINE 訊息。你的回覆必須控制在 600-800 個中文字之間。這是硬性限制，不可以超過 800 字。用 3-5 個段落。寧可講一個重點講得深入透徹，也不要講太多重點但每個都淺。最重要的是：一定要把話講完，最後要有一個完整的結尾句，給學員一個可以帶走的行動建議或鼓勵。絕對不能話講到一半就結束。
