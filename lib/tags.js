...
只輸出摘要文字，不要加標題或格式符號。`;
  ...
  try {
    // 優先 Claude Haiku，Gemini fallback
    let summary = await callClaudeHaiku(prompt, userId, 'update_coach_summary');
...
只輸出旅程摘要文字，不要加標題或格式符號。`;
  ...
  try {
    // 優先 Claude Haiku，Gemini fallback
    let journey = await callClaudeHaiku(prompt, userId, 'update_coach_journey');
