...
const MAX_TOPICS = 20;
const JOURNEY_MIN_DAYS = 3;  // 距上次更新 ≥ 3 天才觸發
const JOURNEY_MIN_TOPICS = 5; // 至少 5 次新對話才值得更新
...
/**
 * 檢查是否該觸發趨勢更新（距上次 ≥ 3 天 + 至少 5 次新對話）
 * 跟 journey 同頻率 — 都是 AI 讀的中間產物，不需要高頻更新
 */
export async function shouldUpdateTrend(userId) {
  const count = await getTopicCount(userId);
  if (count < JOURNEY_MIN_TOPICS) return false;
  ...
  try {
    ...
    if (lastUpdate) {
      ...
      if (daysSince < JOURNEY_MIN_DAYS) return false;
    }
    await r.set(key, String(Date.now()), { ex: 86400 * 7 });
  } catch (_) {}
  return true;
}
...
/**
 * 檢查是否該觸發旅程摘要更新（距上次 ≥ 3 天 + 至少 5 次新對話）
 * 高頻用戶（如佳欣 20-30 次/天）：insight 即時覆蓋，journey 3 天壓縮一次夠了
 * 低頻用戶：內容不夠，更新也沒意義
 */
