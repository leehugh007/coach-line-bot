...
async function syncTagToSupabase(userId, tags) {
  ...
  const { error } = await sb.from('coaching_tags').insert({
    user_id: userId,
    tag: `${tags.topic}:${tags.emotion}:${tags.progress_signal}`,
    source: 'intent',
  });
...
async function restoreTopicsFromSupabase(userId, count) {
  ...
  try {
    ...
    // 解析 tag 格式 "topic:emotion:progress_signal"
...
/**
 * 檢查是否該觸發旅程摘要更新（每 10 次對話）
 */
export async function shouldUpdateJourney(userId) {
  const count = await getTopicCount(userId);
  return count >= JOURNEY_INTERVAL && count % JOURNEY_INTERVAL === 0;
}
...
export async function updateJourneySummary(userId) {
  const [previousJourney, topics, recentSummary] = await Promise.all([
  ...
  const coreIssues = topics.map(t => t.core_issue).filter(Boolean);
  const progressSignals = topics.map(t => `${t.topic}(${t.progress_signal})`).join('、');
...
像一位真人教練在學員檔案上寫的紀錄：記得他怎麼開始、遇過什麼困難、怎麼克服、有什麼進步。

撰寫原則：
1. 用時間軸敘事：從開始 → 中間的挑戰 → 目前的狀態
2. 記錄關鍵轉折：突破、挫折、態度轉變
3. 具體但精煉：「從不敢外食到能自在點餐」比「飲食觀念改善」好
4. 保持客觀溫暖：像在跟同事交接學員檔案
5. 500-800 字${previousSection}

【最近對話趨勢（近況摘要）】：
${recentSummary || '無'}

【最近 ${topics.length} 次對話標籤】：
${progressSignals}

【核心問題】：${coreIssues.slice(-8).join('、')}
