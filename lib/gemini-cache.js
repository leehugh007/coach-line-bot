...
export async function getOrCreateCache(cacheKey, model, systemPrompt) {
  const enabled = process.env.CONTEXT_CACHE_ENABLED;
  if (enabled !== 'true') {
    console.log(`[Cache] Disabled (CONTEXT_CACHE_ENABLED=${enabled})`);
    return null;
  }
  console.log(`[Cache] Enabled, checking Redis for ${cacheKey}...`);

  const r = getRedis();
  const redisKey = `gemini-cache:${cacheKey}`;
