...
async function restoreUserFromSupabase(userId) {
  ...
  try {
    ...
    if (data.intro) {
      const extracted = await extractProfile(data.intro);
      if (extracted) {
...
function normalizeName(name) {
  ...
  return name
    .replace(/\s+/g, '')           // 移除空格
    .replace(/[（）()【】\[\]]/g, '') // 移除括號
    .replace(/[\u{1F000}-\u{1FFFF}]/gu, '') // 移除 emoji
    .toLowerCase()
    .trim();
}
...
export async function batchImportIntros(students) {
  ...
  console.log(`[User] Batch imported ${imported} intros, skipped ${skipped}, duplicates ${duplicates}`);
 ...
 */
export async function findPreloadedIntro(displayName) {
  const r = getRedis();
  ...
  if (exact) return typeof exact === 'string' ? JSON.parse(exact) : exact;
 ...
 */
export async function findPreloadedByRealName(realName) {
  const r = getRedis();
 ...
 * 用真實姓名完成比對（重名時用）
 */
export async function tryMatchByRealName(userId, realName) {
  const preloaded = await findPreloadedByRealName(realName);
 ...
 */
export async function tryMatchPreloaded(userId, displayName) {
  const preloaded = await findPreloadedIntro(displayName);
  if (!preloaded || preloaded.matched) return false;
 ...
 */
export function buildUserContext(user, coachingSummary = null, journeySummary = null, slices = null) {
  if (!user && !coachingSummary && !journeySummary) return '';
  ...
  // === coaching_trend 切片 ===
  if (shouldInject('coaching_trend') && coachingSummary) {
    context += `\n\n【學員心態趨勢 — 根據過去對話累積的觀察】
${coachingSummary}
...
- 根據學員的生活狀況（工作、家庭），給出符合實際的建議`;
  ...
  // === journey 切片 ===
  if (shouldInject('journey') && journeySummary) {
    context += `\n\n【學員旅程 — 從開始到現在的完整故事】
${journeySummary}

根據上面的旅程：
- 提到學員過去克服過的挑戰時，可以引用他的經歷（「你之前也遇過...後來你...」）
- 看到重複出現的模式，溫和地點出來
- 對比學員的成長（「跟剛開始比，你現在...」）`;
  }
  ...
  // log 切片使用情況
  if (slices) {
    console.log(`[UserContext] Slices: identity(always) + ${slices.join(', ') || 'none'} → ${context.length} chars`);
  }
