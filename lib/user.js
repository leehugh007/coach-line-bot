...
export async function findPreloadedIntro(displayName) {
  ...
  // 如果精確比對失敗，嘗試用 SCAN 找包含關係
  try {
    let cursor = 0;
    do {
      const result = await r.scan(cursor, { match: `${PRELOAD_PREFIX}*`, count: 50 });
      cursor = result[0];
      for (const key of result[1]) {
        const storedName = key.replace(PRELOAD_PREFIX, '');
        // 檢查是否有包含關係（例如 "Annie" 包含在 "Annie Chen" 中）
...
export async function getPreloadedStatus() {
  ...
  try {
    ...
    // 先收集所有 key
    do {
      const result = await r.scan(cursor, { match: `${PRELOAD_PREFIX}*`, count: 100 });
    ...
    for (const key of keys) {
      try {
        ...
        results.push({
          lineName: record.lineName || key.replace(PRELOAD_PREFIX, ''),
        ...
        console.error(`[User] Error reading key ${key}:`, e);
      }
    }
  } catch (err) {
