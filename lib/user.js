...
export async function performClassChange(userId, newClassName, opts = {}) {
  ...
  try {
    ...
    if (!skipCacheInvalidation) {
      try {
        await Promise.all([
          r.del(`coach-portrait:${userId}`),
          r.del(`coach-portrait-ver:${userId}`),
          r.del(`coach-renewal-notify-cd:${userId}`),
          r.del(`coach-renewal-push:${userId}:w10`),
          r.del(`coach-renewal-push:${userId}:w11`),
          r.del(`coach-renewal-push:${userId}:w12`),
        ]);
      } catch (err) {
        console.error(`[ClassChange] Cache invalidation error ${userId?.substring(0, 8)}:`, err.message);
