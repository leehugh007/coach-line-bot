...
export async function batchImportIntros(students) {
  ...
  // === 2026-07-06：一次撈全部 users 建正規化名稱 map（供自動換班比對）===
  // 舊版用 display_name.eq 原始字串 + intro.ilike 逐筆查 DB：
  // ① 大小寫/空格/emoji 差一點就 miss（04-22 backfill 實測 140 人漏 8 人）
  // ② intro.ilike 會誤配（名字出現在別人自介裡）
  // DB 端同正規化名多人 → 標 ambiguous 跳過自動配對（配錯人比漏配嚴重，留人工）
  const userByNormName = {};
  try {
    const sbMap = getSupabase();
    if (sbMap) {
      const { data: allUsers } = await sbMap.from('users')
        .select('id, class_name, display_name')
        .limit(5000);
      for (const u of allUsers || []) {
        const n = normalizeName(u.display_name || '');
        if (!n) continue; // 純 emoji/符號名稱正規化後為空，跳過避免互相誤配
        if (userByNormName[n]) userByNormName[n].ambiguous = true;
        else userByNormName[n] = { user: u, ambiguous: false };
      }
    }
  } catch (mapErr) {
    console.error('[User] Build user map error:', mapErr.message);
  }
  ...
  for (const s of studentsToImport) {
    ...
    if (s.className) {
      try {
        const sb = getSupabase();
        if (sb) {
          // 2026-07-06：改用正規化名稱 map 比對（lineName 優先、realName 備援）
          // 舊版 display_name.eq 原始字串 + intro.ilike 的兩個洞見 map 建立處註解
          let matched = false;
          let matchedUserId = null;
          const candidates = [normalizeName(s.lineName), s.realName ? normalizeName(s.realName) : '']
            .filter(Boolean);

          for (const n of candidates) {
            const entry = userByNormName[n];
            if (!entry) continue;
            if (entry.ambiguous) {
              console.log(`[User] Ambiguous skip（DB 多人同正規化名）: ${s.lineName}`);
              break; // 配錯人比漏配嚴重，留人工處理
            }
            matched = true;
            matchedUserId = entry.user.id;
            if (entry.user.class_name !== s.className) {
              // 走 performClassChange — 一次 helper call 統一所有 storage 同步
              // （Supabase 5 欄位 + Redis profile + 6 個 cache key）
              const result = await performClassChange(entry.user.id, s.className, { source: 'auto' });
              if (result.changed) {
                entry.user.class_name = s.className; // map 同步，防同批重複換班
                reassigned++;
                console.log(`[User] Auto reassigned ${entry.user.display_name || entry.user.id.substring(0, 8)} → ${s.className}`);
              }
            }
            break; // 比對到就不繼續
          }
 ...
 * @param {string[]|null} facts - 永久事實（學員糾正過的事，永遠注入、不受 slices 過濾）
 * @returns {string} 要注入的上下文文字
 */
export function buildUserContext(user, recentTags = null, slices = null, activeGoal = null, facts = null) {
  ...
  if (Array.isArray(facts) && facts.length > 0) {
    context += `\n\n【永久注意事項 — 這位學員的長期事實，給建議時要遵守】`;
    for (const f of facts) context += `\n- ${f}`;
    context += `\n→ 給建議前先檢查有沒有踩到上面任何一條。自然遵守就好，不要主動提起「你糾正過我」「你之前說過」這類話，也不用為此道歉——就當你本來就知道，像朋友本來就記得一樣。`;
  }
...
// ====== 永久事實：學員糾正過的事實（2026-07-06 蘿蔔乾/便利商店事件）======
// 糾正偵測只刪錯誤記憶不夠 — Tier1 知識注入是台灣中心（推超商），刪了還會長回來。
// 必須正向記住「學員不吃 X」「住美國沒有超商」這類事實，永遠注入 context。
// v1 只存 Redis（本 session 無 Supabase DDL 權限）；TODO: MCP 授權後補 users 表欄位做雙層。

const FACTS_KEY = (uid) => `coach-facts:${uid}`;
const FACTS_MAX = 12;
...
export async function getUserFacts(userId) {
  try {
    const r = getRedis();
    const facts = await r.get(FACTS_KEY(userId));
    return Array.isArray(facts) ? facts : [];
  } catch (err) {
    console.error('[Facts] getUserFacts error:', err.message);
    return [];
  }
}

/**
 * 新增永久事實（去重 + 上限 FACTS_MAX，滿了淘汰最舊的）
 * @param {string[]} newFacts - 糾正偵測 AI 產出的事實句
 */
export async function addUserFacts(userId, newFacts) {
  if (!Array.isArray(newFacts) || newFacts.length === 0) return;
  try {
    const r = getRedis();
    let facts = await getUserFacts(userId);
    for (const raw of newFacts) {
      if (typeof raw !== 'string') continue;
      const fact = raw.trim();
      if (fact.length < 4 || fact.length > 60) continue; // 太短沒資訊、太長是 AI 在寫作文
      // 去重：完全相同或互相包含都算重複
      if (facts.some(f => f === fact || f.includes(fact) || fact.includes(f))) continue;
      facts.push(fact);
      console.log(`[Facts] Added for ${userId?.substring(0, 8)}: ${fact}`);
    }
    if (facts.length > FACTS_MAX) facts = facts.slice(-FACTS_MAX);
    await r.set(FACTS_KEY(userId), facts);
  } catch (err) {
    console.error('[Facts] addUserFacts error:', err.message);
  }
}

/**
 * 糾正清理（共用 helper）— route.js 即時糾正偵測 + smart-push reconcile 兜底都走這裡
 *
 * 清四層：
 * 1. Supabase coaching_tags：刪含關鍵字的整筆
 * 2. Supabase goals：active 且 goal_text/context 含關鍵字 → 標 replaced
 * 3. Redis coach-goal:{uid}：goal_text/context 含關鍵字 → del（2026-07-06 根因：
 *    之前只標 Supabase replaced，Redis 快取無 TTL 且 getActiveGoal Redis 優先 → 錯誤目標永生）
 * 4. Redis coach:{uid}:topics/summary/journey 快取
 * 另外：舊 facts 含被糾正的關鍵字 → 一併移除（糾正-糾正機制，錯誤事實有回收路）
 *
 * @param {string} userId
 * @param {string[]} topics - AI 判定被永久糾正的關鍵字
 */
export async function cleanCorrectedMemory(userId, topics) {
  if (!Array.isArray(topics) || topics.length === 0) return;
  const sb = getSupabase();
  const r = getRedis();

  for (const topic of topics) {
    console.log(`[Correction] Cleaning: "${topic}" for ${userId?.substring(0, 8)}`);

    // 1. Supabase coaching_tags
    if (sb) {
      try {
      ...
      // 2. Supabase goals（goal_text 或 context 含關鍵字都算）
      try {
        await sb.from('goals')
          .update({ status: 'replaced' })
          .eq('user_id', userId)
          .eq('status', 'active')
          .or(`goal_text.ilike.%${topic}%,context.ilike.%${topic}%`);
      } catch (e) { /* non-fatal */ }
    }
  }

  // 3. Redis coach-goal 快取：goal_text 或 context 命中任一關鍵字 → del
  try {
    const cachedGoal = await r.get(GOAL_KEY(userId));
    if (cachedGoal) {
      const goalText = `${cachedGoal.goal_text || ''} ${cachedGoal.context || ''}`;
      if (topics.some(t => goalText.includes(t))) {
        await r.del(GOAL_KEY(userId));
        console.log(`[Correction] Redis goal deleted (matched correction): ${cachedGoal.goal_text}`);
      }
    }
  } catch (e) { /* non-fatal */ }
  ...
  try {
    await Promise.allSettled([
      r.del(`coach:${userId}:topics`),
      r.del(`coach:${userId}:summary`),
      r.del(`coach:${userId}:journey`),
    ]);
  } catch (e) { /* non-fatal */ }

  // 5. 舊 facts 含被糾正關鍵字 → 移除（學員改口時錯誤事實不會永生）
  try {
    const facts = await getUserFacts(userId);
    if (facts.length > 0) {
      const kept = facts.filter(f => !topics.some(t => f.includes(t)));
      if (kept.length !== facts.length) {
        await r.set(FACTS_KEY(userId), kept);
        console.log(`[Correction] Removed ${facts.length - kept.length} stale facts`);
      }
    }
  } catch (e) { /* non-fatal */ }
}
