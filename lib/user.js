...
async function restoreUserFromSupabase(userId) {
  ...
  try {
    ...
    const profile = {
      ...
      className: data.class_name || null, // snake → camel 轉換（route.js L1129 讀 camelCase）
      info: {
      ...
      // 續報欄位 snake → camel 轉換（契約_續報記錄.md §2）
      renewalIntent: data.renewal_intent || null,
      renewalIntentAt: data.renewal_intent_at || null,
      renewalIntentSource: data.renewal_intent_source || null,
      renewalConfirmedAt: data.renewal_confirmed_at || null,
    };
...
async function syncUserToSupabase(userId, profile) {
  ...
  const row = {
    ...
    // 續報欄位：undefined 時不寫入（防覆蓋 DB，契約 §10 Phase 1.5.A）
...
export async function batchImportIntros(students) {
  ...
  for (const s of studentsToImport) {
    ...
    // === 續報自動換班：如果 Supabase 已有這個學員，直接更新 class_name + 清 renewal 欄位 ===
    // 契約_續報記錄.md §5 T7 觸發源 A：換班 = 新班期，清 renewal 4 欄
    if (s.className) {
      try {
        const sb = getSupabase();
        if (sb) {
              ...
              for (const user of existing) {
                ...
                if (user.class_name !== s.className) {
                  await sb.from('users')
                    ...
                    .eq('id', user.id);
                  // 同步 Redis profile（camelCase 標準 key + 清舊的 snake_case 殘留）
                  const redisKey = `coach-user:${user.id}`;
                  const cached = await r.get(redisKey);
                  if (cached && typeof cached === 'object') {
                    cached.className = s.className;
                    delete cached.class_name; // 清舊 snake_case 殘留（歷史寫入）
                  ...
                  await r.del(`coach-portrait:${user.id}`);
                  await r.del(`coach-portrait-ver:${user.id}`);
                  await r.del(`coach-renewal-notify-cd:${user.id}`);
                  await r.del(`coach-renewal-push:${user.id}:w10`);
                  await r.del(`coach-renewal-push:${user.id}:w11`);
                  await r.del(`coach-renewal-push:${user.id}:w12`);
                  reassigned++;
...
export async function getClassStatus(className) {
  ...
  const r = getRedis();
  const data = await r.get(`coach-class:${className}`);
  if (!data) return null;

  const parsed = typeof data === 'string' ? JSON.parse(data) : data;
  if (!parsed.endDate) return 'active'; // 沒設結束日 = 永遠有效

  const now = new Date();
  const endDate = new Date(parsed.endDate);
  const daysSinceEnd = Math.floor((now - endDate) / 86400000);

  if (daysSinceEnd < 0) return 'active';       // 還沒結業
  if (daysSinceEnd <= 3) return 'graduating';   // 結業後 0-3 天
  if (daysSinceEnd <= 10) return 'grace';       // 寬限期 4-10 天
  return 'expired';                              // 超過寬限期
}
