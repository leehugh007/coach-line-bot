...
export async function batchImportIntros(students) {
  ...
  for (const s of students) {
    if (!s.lineName || !s.intro) { skipped++; continue; }

    const normalizedName = normalizeName(s.lineName);
    if (!normalizedName) { skipped++; continue; }

    const record = {
      lineName: s.lineName,        // 原始名稱
      className: s.className || null,
      studentId: s.studentId || null,
      intro: s.intro,
      note: s.note || null,
      importedAt: new Date().toISOString(),
      matched: false,              // 尚未配對到 userId
    };

    await r.set(`${PRELOAD_PREFIX}${normalizedName}`, record);
    await r.sadd(PRELOAD_INDEX, normalizedName); // 加入索引
    imported++;
  }

  console.log(`[User] Batch imported ${imported} intros, skipped ${skipped}`);
